...
public class LoginActivity extends BaseActivity {
    ...
    private void initData() {
        SharedPreferences userSettings = getApplicationContext().getSharedPreferences("user", 0);
        String name = userSettings.getString("account","");
    ...
    private void tryLogin(final String account, final String password) {
        new AsyncTask<Void, Void, Integer>() {
            ...
            protected Integer doInBackground(Void... params) {
                try {
                    ...
                    if (!mNetService.isConnected()) {
                        System.out.println("!mNetService.isConnected()");
                        return 0;
                    }
                    ...
                    data.start();
                    System.out.println("qintest 获取登陆信息返回了");
                    if (data.isError) {
                        data.isError = false;
                        return 0;
                    }
            ...
            protected void onPostExecute(Integer result) {
                ...
                if (result == 0) {
                    showCustomToast("服务器异常,请稍后重试");
                } else {
                    if (result == 2) {
                        showCustomToast("登录失败");
                    } else if (result == 1) {
                        saveUserInfo(mAccount.getText().toString(), mPassword.getText().toString());
