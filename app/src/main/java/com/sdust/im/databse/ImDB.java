...
//消息页数据库管理类
public class ImDB {
	public static String DB_NAME = "im_local";
	...
	private ImDB(Context context) {
		System.out.println("qintest uid " +  ApplicationData.getInstance().getUserInfo().getId());
		ImOpenHelper imOpenHelper = new ImOpenHelper(context, DB_NAME + ApplicationData.getInstance().getUserInfo().getId(),
				null, VERSION);
